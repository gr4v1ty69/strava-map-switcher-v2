/*
 * Map switcher 2026 for Strava website.
 * MIT License.
 */
...
/* --------------------------------------------------------------------- */
/* Activity pages: global hook for new <select>                          */
/* --------------------------------------------------------------------- */
...
MapSwitcher.wait(function () {
	...
	console.log("MapSwitcher: activity select found (global)", mapTypeSelect);
	mapTypeSelect = jQuery(mapTypeSelect);

	// Map Strava's numeric values to layer IDs
	const valueToLayer = {
		"0": "standard",
		"5": "satellite",
		"4": "googlehybrid",
	...
	// Patch changeMapType once to inject extra layers/Pegman
	if (Strava && Strava.Maps && Strava.Maps.Mapbox && Strava.Maps.Mapbox.CustomControlView) {
		...
		if (!proto.__mapSwitcherPatched) {
			...
			proto.changeMapType = function (selectedMapTypeId) {
				...
				if (once) {
					once = false;
					console.log("MapSwitcher: first changeMapType (global), injecting layers/Pegman");
					addLayers(map);
					if (map.instance) {
	...
	console.log("MapSwitcher: activity options after append (global):", mapTypeSelect.html());
	...
	// Restore preference if option exists
	...
	if (preferred) {
		...
		if (mapTypeSelect.find(`option[value="${selectValue}"]`).length) {
			console.log("MapSwitcher: selecting preferred map (global)", preferred, "as", selectValue);
			mapTypeSelect.val(selectValue).trigger('change');
		}
	}
}, function (err) {
	console.log("MapSwitcher: global activity wait() failed", err);
});

/* --------------------------------------------------------------------- */
/* Segment Explorer (keep original arrive-based logic)                   */
/* --------------------------------------------------------------------- */
